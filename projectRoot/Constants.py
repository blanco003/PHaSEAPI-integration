...
- If the user doesnt like the recipe and specifies that he want to add new costraints to get a new suggestion, print the string "TOKEN 2.05", then print a JSON with both the information previously collected and add the new one by adding them together. Include every field, but set the absent information as an empty string (for atomic fields) or an empty list (for list fields).
